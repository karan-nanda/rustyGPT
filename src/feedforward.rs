...
impl FeedForward {
    ...
    pub fn new(embedding_dim:usize, hidden_dim: usize) -> Self {
        ...
        FeedForward {
            ...
            b1 : Array2::zeros((1, hidden_dim)), //Bias init to 0
            w2: Array2::from_shape_fn((hidden_dim, embedding_dim), |_| normal_w2.sample(&mut rng)),
            b2: Array2::zeros((1, embedding_dim)), // Bias init to 0
            input: None,
...
impl Layer for FeedForward {
    ...
    fn backward (&mut self, grads: &Array2<f32>, lr: f32) -> Array2<f32> {
        ...
        //Gradients for hidden_post_activation
        ...
        // Gradient for input (after feedforward computation)
        let grad_input_feedforward = grad_hidden_pre_activation.dot(&self.w1.t());

        //Add the gradients from residual connections
        // Forward pass : output = w2(ReLU(w1* input + b1)) + b2 + input
        // Backward pass : grad_input = grad_feedforward + grad_residual

        let grad_input = grad_input_feedforward + grads ; 


        // Update parameters via Adam optim
    ...
    fn forward(&mut self, input : &Array2<f32>) -> Array2<f32> {
        ...
        output + input //residual connection (No need for a layerNorm)
    }  
}
